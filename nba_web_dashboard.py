...
fd_pivot = fd_pivot.rename(columns={
    'Points': 'FD Points',
    'Assists': 'FD Assists',
    'Rebounds': 'FD Rebounds'
})
...
# Pace projections
if 'Points' in merged.columns and 'Minutes' in merged.columns:
    merged['Paced Points'] = (merged['Points'] / merged['Minutes'].replace(0, 0.1)) * 36
else:
    merged['Paced Points'] = np.nan
if 'Assists' in merged.columns and 'Minutes' in merged.columns:
    merged['Paced Assists'] = (merged['Assists'] / merged['Minutes'].replace(0, 0.1)) * 36
else:
    merged['Paced Assists'] = np.nan
if 'Rebounds' in merged.columns and 'Minutes' in merged.columns:
    merged['Paced Rebounds'] = (merged['Rebounds'] / merged['Minutes'].replace(0, 0.1)) * 36
else:
    merged['Paced Rebounds'] = np.nan
...
st.dataframe(
    merged[available_columns].dropna(how='all', axis=1),
